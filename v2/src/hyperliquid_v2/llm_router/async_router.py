...
class AsyncModelRouter:
    def __init__(
        ...
        self.policy = policy
        self.fallback = DeterministicShadowProvider()

    async def decide(self, packet: DecisionPacket) -> RoutedDecision:
        primary_failed = False
        try:
            primary = await self.primary.decide(packet)
        except Exception:  # noqa: BLE001
            LOGGER.exception("Primary LLM failed; using deterministic shadow fallback")
            primary = await self.fallback.decide(packet)
            primary_failed = True

        challenger = None
        if self.challenger and self._needs_challenger(packet, primary):
            try:
                challenger = await self.challenger.decide(packet)
            except Exception:  # noqa: BLE001
                LOGGER.exception("Challenger LLM failed; continuing conservatively")

        routed = resolve(packet, primary, challenger)
        if primary_failed:
            return RoutedDecision(
                final_action=routed.final_action,
                source="provider_failure_fallback",
                primary=routed.primary,
                challenger=routed.challenger,
                reason="primary_provider_failed:" + routed.reason,
            )
        return routed

    def _needs_challenger(self, packet: DecisionPacket, primary: ModelDecision) -> bool:
        high_impact = (
...
def resolve(
    ...
    if primary.action not in allowed:
        return RoutedDecision(
            _fallback(packet), "risk_contract", primary, challenger,
    ...
    if challenger is None:
        return RoutedDecision(
            primary.action, "primary", primary, None,
    ...
    if primary.action == challenger.action:
        return RoutedDecision(
            primary.action, "agreement", primary, challenger,
    ...
    if DecisionAction.OPEN in {primary.action, challenger.action}:
        ...
        return RoutedDecision(
            fallback, "conservative_resolver", primary, challenger,
    ...
    close_candidates = [
        ...
        and decision.expected_value_close_r >= decision.expected_value_hold_r
    ]
    if close_candidates and DecisionAction.CLOSE in allowed:
        return RoutedDecision(
            DecisionAction.CLOSE, "economic_resolver", primary, challenger,
            "close_value_dominates",
        )
    return RoutedDecision(
        _fallback(packet), "conservative_resolver", primary, challenger,
...
def _sizing_within_contract(packet: DecisionPacket, decision: ModelDecision) -> bool:
    if decision.action is not DecisionAction.OPEN:
        return True
    envelope = packet.risk_envelope
    return (
        decision.selected_leverage in envelope.allowed_leverage
        and decision.selected_effective_exposure is not None
        and 0 < decision.selected_effective_exposure <= envelope.maximum_effective_exposure
        and decision.selected_balance_portion is not None
        and 0 < decision.selected_balance_portion <= envelope.maximum_balance_portion
        and decision.selected_stop_distance_pct is not None
